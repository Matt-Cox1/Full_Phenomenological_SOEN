...
# Filename: treads/training_thread.py

"""
This file implements the TrainingThread class, which is responsible for managing the training process
of a neural network model in a separate thread. It allows for asynchronous training that doesn't block the main GUI thread.
"""
...
class TrainingThread(QThread):
    ...
    def run(self):
        self.log_signal.emit("Training thread started running")
        ...
        try:
            ...
            self.log_signal.emit(f"Error in training thread: {str(e)}")
            self.error_signal.emit(str(e))
        finally:
            self.set_state(TrainingState.STOPPED)
        self.log_signal.emit("Training thread finished")
